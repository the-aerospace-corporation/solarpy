...
class OutputCellTechRDCdata(object):
        ...
        for i, param in enumerate(self.params):
            if param in rdc.remainingFactorsAvailable:
                if getattr(rdc, param):
                    ...
                    rdc_dataframe_json['energy_mev'] = rdc_curve[:, 0]
                    rdc_dataframe_json[self.rfs_json[i]] = rdc_curve[:, 1]
    ...
    def proton_to_electron_conversion_data_frame(self, proton_rdc, electron_rdc):
        p_to_e_dataframe = pd.DataFrame(columns=self.rfs)
        p_to_e_dataframe_json = pd.DataFrame(columns=self.rfs_json)
        for i, param in enumerate(self.params):
            if getattr(proton_rdc, param) and getattr(electron_rdc, param):
                rdc_p = getattr(proton_rdc, param)
                rdc_e = getattr(electron_rdc, param)
                proton_remaining_factor = rdc_p.get_fluence_at_reference(rdc_p.critical_remaining_factor)
                electron_remaining_factor = rdc_e.get_fluence_at_reference(rdc_e.critical_remaining_factor)
                proton_to_electron_factor = [electron_remaining_factor / proton_remaining_factor]
                p_to_e_dataframe[param] = proton_to_electron_factor
                p_to_e_dataframe_json[self.rfs_json[i]] = proton_to_electron_factor
        return p_to_e_dataframe
    ...
    def output_all(self, filename):
        ...
        if self.electron_rdc_object and self.proton_rdc_object:
            p_to_e_dataframe = self.proton_to_electron_conversion_data_frame(self.proton_rdc_object, self.electron_rdc_object)
        ...
        with pd.ExcelWriter(filename_xlsx) as writer:
            ...
            p_to_e_dataframe.to_excel(writer, sheet_name='Proton-To-Electron', index=False)

            rdc_cell_properties = pd.DataFrame([shield_thickness_in_cm],index=['Shield thickness in cm'])
        ...
        json_build = {'electron_rdcs_unidirectional': electron_u_rdc_dataframe.to_dict(),
                      ...
                      'proton_to_electron': p_to_e_dataframe.to_dict(),
